...
class ApplicationsPaginationMenu:
    ...
    def create(self, applications: List[Application], page: int):
        ...
        backward_button: InlineKeyboardButton = InlineKeyboardButton(
            '<<',
            callback_data=CallbackType.APPLICATIONS_PAGINATION_BACKWARD)
        forward_button = InlineKeyboardButton(
            '>>',
            callback_data=CallbackType.APPLICATIONS_PAGINATION_FORWARD)
        back_button = InlineKeyboardButton(
            self.localization.lang['to_main_menu'],
            callback_data=CallbackType.APPLICATIONS_PAGINATION_BACK)
        for app in applications:
            button_text: str = self._assemble_button_text(app)
            callback_data: str = CallbackType.APPLICATION_PAGINATION_APP + '_' + str(app.id)
